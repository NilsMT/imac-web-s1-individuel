## L'organisation des dossiers :

```
- docs : les documents
- src : le code
    - assets
        - fonts : les polices
        - img : les images
    - css
        - components : les règles css qui sont répétées pour des "composants"
        - pages : le css des pages individuelles
    - html : les pages HTML
```

> ℹ️ | la page [index.html](./src/index.html) correspond à la page d'accueil

> ⚠️ | Aucun code CSS/HTML n'a été généré par IA ni copié depuis le PenPot (à l'exceptions des textes)

> Réalisé en `4h 49m 07s` <sub>oui j'ai chronométré</sub><br>
> Mise à jour sémantique en ~`32m`<br>
> Temps total : ~`5h 21m 07s`

Nils MOREAU--THOMAS
